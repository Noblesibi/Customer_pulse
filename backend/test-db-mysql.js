...
import { db, initializeDatabase } from './src/config/mysql.js';

dotenv.config();

console.log('🏁 Starting MySQL / Percona Server Adapter verification test...');
console.log(`Host: ${process.env.DB_HOST || 'localhost'}`);
console.log(`Database: ${process.env.DB_DATABASE || 'customer_pulse'}`);
...
async function runTests() {
  try {
    ...
    console.log('\n🎉 ALL DATABASE TESTS PASSED SUCCESSFULLY! The MySQL / Percona Server Firestore-adapter is working perfectly.');
    process.exit(0);
  } catch (err) {
